...
# remove sparse terms to reduce dataset size
merged_dfm<-TermDocumentMatrix(merged_corpus)
merged_corpus<-as.Corpus(removeSparseTerms(merged_dfm,.75))

MyTokenizer<- function(x, n) {NGramTokenizer(x, Weka_control(min = n, max = n))}

# Build n-grams (1-5)

mc1 <- MyTokenizer(merged_corpus, 1)
...
mc2 <- MyTokenizer(merged_corpus, 2)
...
mc3 <- MyTokenizer(merged_corpus, 3)
...
mc4 <- MyTokenizer(merged_corpus, 4)
...
mc5 <- MyTokenizer(merged_corpus, 5)
